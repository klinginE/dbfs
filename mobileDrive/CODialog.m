...
- (void)layoutComponents {
  ...
  if (self.title.length > 0) {
      titleHeight = [self sizeOfString:self.title withFont:self.titleFont].height;/*[self.title sizeWithFont:self.titleFont
  ...
  if (self.subtitle.length > 0) {
      subtitleHeight = [self sizeOfString:self.subtitle withFont:self.subtitleFont].height;/*[self.subtitle sizeWithFont:self.subtitleFont
                               constrainedToSize:CGSizeMake(layoutWidth, MAXFLOAT)
                                   lineBreakMode:UILineBreakModeWordWrap].height;*/
    minY += kCODialogPadding;
  }
